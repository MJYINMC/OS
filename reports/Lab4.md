...
​		和问题1解答中类似，如果我们不进行等值映射，那么马上下一条指令取指会发生错误，产生exception，而在opensbi引导的情况下，这个异常是在M态进行处理的，并不会代理到		S态，因此我们无法通过trap_handler捕捉到这个异常。一种思路是在异常处理程序中，手动设置xepc（如果让这个异常代理到S态，那就是sepc）的值为PC+offset+4，再通过xret		PC被将被写为虚拟地址中的下一条指令，这样就不需要做等值映射。