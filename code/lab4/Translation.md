...
Let pte be the value of the PTE at address a?+?va.vpn[i]?¡Á?PTESIZE. (For Sv32, PTESIZE=4.) If accessing pte violates a PMA or PMP check, raise an access-fault exception corresponding to the original access type.

If pte.v?=?0, or if pte.r?=?0 and pte.w?=?1, stop and raise a page-fault exception corresponding to the original access type.

Otherwise, the PTE is valid. If pte.r?=?1 or pte.x?=?1, go to step 5. Otherwise, this PTE is a pointer to the next level of the page table. Let i?=?i???1. If i?<?0, stop and raise a page-fault exception corresponding to the original access type. Otherwise, let a?=?pte.ppn?¡Á?PAGESIZE and go to step 2.
...
If i?>?0 and pte.ppn[i???1?:?0]?¡Ù?0, this is a misaligned superpage; stop and raise a page-fault exception corresponding to the original access type.
